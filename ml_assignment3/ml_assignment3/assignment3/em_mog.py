...
def m_step(w, X, mu, sigma, phi, k):
    ...
    mu = np.dot(w.T, X) / np.sum(w, axis=0)[:, np.newaxis] #componentwise division
    ...
    # y no work?mu = np.multiply(np.sum(w), np.sum(w, axis=0)) / np.sum(w, axis=0) #if not possible try np.multiply(np.sum(X), np.sum(w)) or np.sum(np.multiply(X,w))
